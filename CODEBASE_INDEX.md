...
| `alembic.ini` / `alembic/` | Migration config + 57 revisions (`alembic/versions/`), head `nat2_gateway_host_check` |
| `database_utils/` | The library package (see below) |
| `tests/` | 28 test files, **243 tests** (`pytest.ini`: `asyncio_mode = auto`; in-memory SQLite). `conftest.py` holds the shared `db` + `plant` fixtures (a real in-memory SQLite network graph: `CORE-1 → OLT-1 → SPL-1 → SPL-2 → ONT-1`, splitters passive, ACTIVATION/SUSPENSION bound per device type). **Cycle 10**: `test_network_graph_model.py`, `test_network_graph_traversal.py`, `test_playbook_binding.py`, `test_provisioning_run.py`, `test_network_graph_migrations.py`, `test_isp_seed_passive.py` (`test_topology_purpose.py` / `test_playbook_topology.py` deleted). Also `test_attested_adoption.py` (ba1 guardrails), `test_client_install_field_drop.py` (cf1 guardrails + single-head file scan). **NAT transport (2026-08-13)**: `test_transport_resolver.py` (`resolve_endpoint()` behaviour), `test_nat_transport_constants.py` (model/migration CHECK-fragment parity, nat1 + nat2), `test_nat_transport_schemas.py` (`NetworkAccessCreate` validator), `test_nat_transport_gateway_check.py` (whole-branch-review fixes: DB CHECK `ck_network_access_nat_gateway_host`, `NetworkAccessUpdate` validator, `resolve_endpoint`'s cross-tenant `access` guard) |
...
| `transport.py` | **2026-08-13, doc 34 R23 rewrite** — `resolve_endpoint(db, item, company_id, default_port, pylon_socks5=None, access=None)`: the one place `InventoryItem` + the tenant's default `kind='olt'` `NetworkAccess` row become a dial `ResolvedEndpoint(host, port, proxy, mode)`. `direct` unchanged; `nat_zt`/`nat_public` target `(access.gateway_host, item.nat_port)`, never `item.mgmt_host`; missing mapping → `NAT_MAPPING_NOT_SET`; `nat_zt` needs a `pylon_socks5` arg that no backend-erp caller supplies yet → `TRANSPORT_UNAVAILABLE` (fails closed, not dial-capable this cycle) |
