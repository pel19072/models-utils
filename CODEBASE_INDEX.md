...
| `alembic.ini` / `alembic/` | Migration config + 42 revisions (`alembic/versions/`) |
| `database_utils/` | The library package (see below) |
| `tests/` | 16 test files, ~90 tests (`pytest.ini`: `asyncio_mode = auto`; in-memory SQLite) |
...
- **Grandfathered verification**: `t2_grandfather_email_verified` (head) — backfills `user.email_verified = TRUE` for all pre-overhaul users (login-gate lockout guard)
