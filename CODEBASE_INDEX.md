...
| `alembic.ini` / `alembic/` | Migration config + 43 revisions (`alembic/versions/`) |
| `database_utils/` | The library package (see below) |
| `tests/` | 17 test files, ~100 tests (`pytest.ini`: `asyncio_mode = auto`; in-memory SQLite) — incl. `test_attested_adoption.py` (ba1 migration/seed/schema guardrails) |
...
- **Brownfield adoption**: `ba1_attested_adoption` (**head**) — additive `client_service` adoption columns + FK + partial index `ix_client_service_adopted` + ADMIN-only `client_services.adopt` permission (MANAGER excluded at both seed auto-grant sites: `isp_seed.ADMIN_ONLY_PERMISSIONS` / `rbac_seed.MANAGER_EXCLUDED_PERMISSIONS`)
